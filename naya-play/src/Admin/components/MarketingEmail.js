...
const MarketingEmailManager = () => {
    ...
    const fetchUserEmails = async () => {
      try {
        ...
        // 1. Have emailOffers explicitly set to true
        // 2. Don't have emailOffers field set at all (default opt-in)
        const emails = snapshot.docs
          .filter(doc => {
            const userData = doc.data();
            // Include if emailOffers is true or if it's not set at all
            return userData.emailOffers !== false;
          })
    ...
    const renderEmailPreview = () => {
      return (
        <div className="bg-[#1a1b1e] text-white p-6 rounded-lg space-y-4">
    ...
    const handleSendEmail = async () => {
      ...
      try {
        ...
        const response = await fetch('https://nayaplay-backend.onrender.com/api/send-marketing-email', {
          method: 'POST',
          headers: {
        ...
        if (result.success) {
          setStatus({
            type: 'success',
            message: `Marketing email sent successfully to ${userEmails.length} users!`
          });
          setSubject('');
          setEmailContent('');
        } else {
    ...
    return (
      <div className="space-y-6 max-w-4xl mx-auto">
        <div className="flex justify-between items-center">
          <div>
            <h2 className="text-2xl font-bold text-white">Marketing Email</h2>
            <p className="text-gray-400">Send emails to opted-in users</p>
          </div>
        ...
        <div className="bg-[#1a1b1e] rounded-xl border border-white/5 p-6 space-y-6">
          {/* Subject Input */}
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-2 bg-[#2d2e33] border border-white/10 rounded-lg 
          ...
          {/* Preview Section */}
          {preview && (
            <div className="border border-white/10 rounded-lg p-4 bg-[#2d2e33]">
          ...
          >
            {loading ? (
              ...
              <>
                <Send size={20} />
                <span>Send Marketing Email</span>
              </>
            )}
          </button>
        </div>
...
export default MarketingEmailManager;